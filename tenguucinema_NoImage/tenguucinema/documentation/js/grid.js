(function($){
    ...
    GX.Views.SingleView = Backbone.View.extend({
        ...
        render: function(){
            var self = this;
            /*
            jQuery.post("post.json.more.php", {  }, function(data){
                var json_data = $.parseJSON(data);
                var _model = new GX.Models.Post(json_data.post);
                self.success(_model);
            });
            */
            ...
            $item.find('video').each(function(){
                if( !$(this).parent().hasClass('mejs-mediaelement') ){
                    $(this).attr("width", $(this).parent().width() )
                           .attr("height", parseInt($(this).parent().width()*270/480));
    ...
    GX.Views.SingleOpenGI = GX.Views.SingleView.extend({
        success: function(_model){
            ...
            if( item_exist ){
                $next.before('<div class="gi-viewport"></div>');
            }
            else{
                $last.after('<div class="gi-viewport"></div>');
            }
            ...
            $parent.find('a.close-viewport').on('click', function(){
                ...
                $parent.find('.gi-viewport').remove();
                // $parent.find('.gi-viewport').slideUp('fast', function(){
                //     $parent.find('.gi-viewport').remove();
                // });
            });
            // prev
        ...
        auto_flip: function(start){
            ...
            if(start){
                ...
                if( typeof flip_index!=='undefined' ){
                    var ci = parseInt( self.$el.parent().attr('data-flip-index') );
    ...
    GX.Views.GridMasonry = GX.Views.GridView.extend({
        ...
        loadItems: function(){
            ...
            this.collection.each(function(model){
                ...
                var $el = postView.render().$el;
                var col = parseInt(self.$el.parent().data('column'));
                $el.addClass('col-xs-12 col-sm-6 col-md-'+parseInt(12/col) );
    ...
    $(window).load(function(){
        ...
        var fix_autoflip_size = function(){
            ...
            $('.grid-container.auto-flip > div').width( parseInt( $('.grid-container.auto-flip').outerWidth()/parseInt($('.grid-container.auto-flip').data('column')) ) * parseInt($('.grid-container.auto-flip').data('column')) );
        };
